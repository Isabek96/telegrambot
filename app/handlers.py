...
async def show_otkrytyj_grunt(callback: CallbackQuery):
    await callback.message.edit_text("Выберите категорию из открытого грунта:", reply_markup=await kb.inline_menu_otk_grunt())
    await callback.answer()

@router.callback_query(F.data == "Закрытый грунт")
async def show_zaktiti_grunt(callback: CallbackQuery):
    await callback.message.edit_text("Выберите категорию из Закрытого грунта:", reply_markup=await kb.inline_menu_zakr_grunt())
    await callback.answer()

@router.callback_query(F.data == 'Удобрения')
async def show_zaktiti(callback: CallbackQuery):
    await callback.message.edit_text("Выберите семена и удобрения из каталога.", reply_markup=await kb.inline_menu_udobrenie())
    await callback.answer()
